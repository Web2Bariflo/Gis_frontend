  ...
  return (
    ...
    <div className='flex flex-col justify-center mb-0 items-center'>
        <img src={logo} alt="logo" className='w-1/4'/>
         <p className='text-md font-semibold leading-tight tracking-tight text-gray-500 md:text-md dark:text-gray-200 text-center' style={{paddingTop: "0px"}}>{t('AquaboticsAccount')}</p>
    </div>

    <div className="flex flex-col items-center mt-6 -mx-2">
        <Imgupload id={userId}/>
        <h4 className="mx-2 mt-2 font-medium text-gray-500 dark:text-gray-200">{user}</h4>
        <p className="mx-2 mt-1 text-sm font-medium text-gray-500 dark:text-gray-200">{userEmail}</p>
    </div>

    <div className="flex flex-col justify-between flex-1 mt-6 text-gray-500">
        <nav>
            <div className={`flex items-center px-4 py-2  ${location.pathname === '/user-dashboard' ? ' bg-gray-200  dark:text-gray-200 text-gray-700' : 'text-gray-500'} rounded-lg dark:bg-gray-800 cursor-pointer`} onClick={()=>navigate('/user-cluster-list')}>
            ...
            <div className={`flex items-center px-4 py-2 mt-5 transition-colors duration-300 transform rounded-lg  hover:bg-gray-200 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700 cursor-pointer ${location.pathname === '/analytics' ? ' bg-gray-200  dark:text-gray-200 text-gray-700' : 'text-gray-500'}`} onClick={()=>navigate('/analytics')}>
            ...
            <div className="flex items-center px-4 py-2 mt-5 transition-colors duration-300 transform rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700 cursor-pointer" onClick={()=>setOpenSettings(!openSettings)}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {
                openSettings ? <div className={`flex items-center px-4 py-2 mt-5  transition-colors duration-300 transform rounded-lg dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700 cursor-pointer  ${location.pathname === '/change-password/' ? ' bg-gray-200  dark:text-gray-200 text-gray-700' : 'text-gray-500'}`} onClick={()=>navigate(`/change-password/${userId}`)}>
                    <i className="fa-solid fa-key"></i>

                    <span className="mx-4 font-medium">{t('changepassword')}</span>
                </div> : null
            }
            <a className="flex items-center px-4 py-2 mt-5  transition-colors duration-300 transform rounded-lg dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700" href="#" onClick={()=>setIsOpen(true)}>
